import { createRelatedChunkExpansion } from './related-chunk-expansion.js';
...
export function createChunkSelection({ host, queries, rules }) {
    ...
    const { extractChatKeywords, buildSearchContext, filterChunksByConditions,
        parseRegistryKey, COLLECTION_PREFIXES, INTERNAL_COLLECTION_IDS } = rules;
    const hashCache = new LRUCache(HASH_CACHE_SIZE);
    const relatedChunks = createRelatedChunkExpansion({ getSavedHashes, getChunkMetadata, log });
    ...
    async function select({ chat, settings, generationType: type, testMessage = null }) {
        ...
        chunks = await relatedChunks.expandSummaries(chunks, settings, debugData);
        ...
        chunks = await relatedChunks.applyLinks(chunks, settings, debugData);
